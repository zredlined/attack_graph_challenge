...
def generate_attack_graph_dataset(num_samples: int = 100, difficulty: str = "simple", path_bias=None):
    ...
    # Define data pools
    CONFIG_FILES = [
        "config.json", "settings.ini", "db.conf", "app.env", "credentials.xml",
        "web.config", "db_conn.properties", "local_settings.py", "secrets.yml"
    ]
    
    CVE_PRODUCTS = ["Nexus", "DataFlow", "AuthServ", "LogForged", "SecureStream", "CoreAPI"]
    USELESS_PORTS = [8080, 8443, 9001, 1337, 3000, 5000]
    USELESS_FILES = ["app.log.old", "temp_notes.txt", "archive.zip", "main.js", "style.css", "logo.png"]
    ...
    config_builder.add_column(
        C.ExpressionColumn(
            name="exploit_cve",
            expr="CVE-2024-{{ cve_product }}-{{ cve_number }}"
        )
    )
    ...
    config_builder.add_column(
        C.ExpressionColumn(
            name="decoy_cve",
            expr="CVE-2024-{{ cve_product }}-Decoy-{{ cve_number }}"
        )
    )
    ...
    if num_samples <= 10:
        ...
        results = data_designer_client.create(config_builder, num_records=num_samples, wait_until_done=True)
...
if __name__ == "__main__":
    ...
    num_samples = 50_000
    difficulty = "complex"
    simple_data = generate_attack_graph_dataset(num_samples=num_samples, difficulty=difficulty)
    
    # Save to file
    df = pd.DataFrame(simple_data)
    df.to_json(f"attack_graph_{difficulty}_{num_samples}.jsonl", orient="records", lines=True)
    ...
    print("\n--- Dataset Stats ---")
    paths = [json.loads(record["initial_state"])["metadata"]["solution_path"] 
